...
def suscribirse(user_id: str, fund_id: str):
    ...
    users_collection.update_one({"id": user_id}, {"$inc": {"balance": -fund['minimum_amount']}})

    send_notification(user_id, f"Suscrito exitosamente al fondo {fund['name']}", "sms")
...
def cancelar(user_id: str, fund_id: str):
    ...
    send_notification(user_id, f"Cancelada la suscripción al fondo {fund['name']}", "email")
